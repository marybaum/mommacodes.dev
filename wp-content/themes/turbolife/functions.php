...
define('CHILD_THEME_NAME', __('TurboLife', 'tf'));
define('CHILD_THEME_URL', 'http://github.com/marybaum');
define('CHILD_THEME_VERSION', '0.1.5');
...
add_filter( 'genesis_post_info', 'tf_post_info_filter' );
function tf_post_info_filter($post_info) {
    $post_info = '[post_edit]';
    return $post_info;
}
...
add_action( 'genesis_before_loop', 'genesis_do_nav' );
...
add_filter('genesis_breadcrumb_args', 'tf_breadcrumb_args');
function tf_breadcrumb_args($args) {
	$args['sep'] = ' &raquo; ';
	$args['list_sep'] = ', ';
	// Genesis 1.5 and later
	$args['display'] = true;
	$args['labels']['prefix'] = '';
	$args['labels']['author'] = __(' ', 'tf');
	$args['labels']['category'] = __(' ', 'tf');
	// Genesis 1.6 and later
	$args['labels']['tag'] = __(' ', 'tf');
	$args['labels']['date'] = __(' ', 'tf');
	$args['labels']['search'] = __('Find ', 'tf');
	$args['labels']['tax'] = __(' ', 'tf');
	$args['labels']['post_type'] = __(' ', 'tf');
	$args['labels']['404'] = __('404', 'tf');
	// Genesis 1.5 and later
	return $args;
}

function tf_body_class_add_categories( $classes ) {
...
add_filter( 'body_class', 'tf_body_class_add_categories' );
...
function tf_enqueue_scripts() {
	...
	wp_register_style('tf_googtype' , 'http://fonts.googleapis.com/css?family=Montez' , array() );
	wp_register_style('tf_arquitecta' , get_stylesheet_directory_uri() . '/type/arquitectaLMBH/arquitecta.css' , array() , '2' );
	wp_register_style('tf_aviano' , get_stylesheet_directory_uri() . '/type/avianosans/avianosans.css' , array(), '2' );
	wp_register_style('tf_parisienne' , get_stylesheet_directory_uri() . '/type/laparisienne/laparisienne.css' , array(), '2' );
	
	wp_enqueue_style('foundation');
	wp_enqueue_style('dashicons');
	wp_enqueue_style('tf_googtype');
	wp_enqueue_style('tf_arquitecta');
	wp_enqueue_style('tf_aviano');
	wp_enqueue_style('tf_parisienne');
...
add_action('genesis_after_entry', 'tf_set_background_image');
function tf_set_background_image() {
...
add_filter('get_the_content_more_link', 'tf_read_more_link');
function tf_read_more_link() {
	return '... <a class="more-link" href="' . get_permalink() . '">More.</a>';
}

// Change search-form input box copy

add_filter('genesis_search_text', 'tf_search_text');
function tf_search_text($text) {
...
add_action('genesis_after_header', 'tf_entry_bgd');
function tf_entry_bgd() {
	if ((	is_singular('post')) && has_post_thumbnail()) {
...
if (is_page('archive')) {
	...
	remove_action('genesis_after_header', 'tf_entry_bgd');
 ...
 */
add_action('genesis_before_entry', 'tf_postimg_above_title');

function tf_postimg_above_title() {

	remove_action('genesis_entry_content', 'genesis_do_post_image', 8);

	add_action('genesis_entry_header', 'tf_postimg', 9);
}
 ...
 * Output gravatar before content on single Posts with Featured image
 add_action( 'genesis_before_content', 'tf_gravatar' );
 function tf_gravatar() {
...
add_filter('comment_form_defaults', 'tf_comment_form_defaults');
function tf_comment_form_defaults($defaults) {

	$defaults['title_reply'] = __('Dish it.');
	return $defaults;
...
genesis_register_sidebar( array(
 'id'     => 'home-top-hero',
 'name'    => __( 'home-top-hero', 'turbolife' ),
 'description' => __( 'This is the hero section of the homepage.', 'turbolife' ),
'before_title'=> __( '<h2>', 'turbolife'),
'after_title' => __( '</h2>', 'turbolife'),
 ) );
 genesis_register_sidebar( array(
 'id'     => 'home-top-list',
 'name'    => __( 'home-top-list', 'turbolife' ),
 'description' => __( 'This part of the homepage is perfect for a list of posts.', 'turbolife' ),
'before_title'=> __( '<h3>', 'turbolife'),
'after_title' => __( '</h3>', 'turbolife'),
 ) );
genesis_register_sidebar( array(
 'id'     => 'homegrid-1',
 'name'    => __( 'Home - homegrid-1', 'turbolife' ),
 'description' => __( 'This is the homegrid-1 section of the homepage.', 'turbolife' ),
 'before_title'=> __( '<h4>', 'turbolife'),
 'after_title' => __( '</h4>', 'turbolife'),
) );
genesis_register_sidebar( array(
 'id'     => 'homegrid-2',
 'name'    => __( 'Home - homegrid-2', 'turbolife' ),
 'description' => __( 'This is the homegrid-2 section of the homepage.', 'turbolife' ),
 'before_title'=> __( '<h4>', 'turbolife'),
 'after_title' => __( '</h4>', 'turbolife'),
) );
genesis_register_sidebar( array(
 'id'     => 'homegrid-3',
 'name'    => __( 'Home - homegrid-3', 'turbolife' ),
 'description' => __( 'This is the homegrid-3 section of the homepage.', 'turbolife' ),
 'before_title'=> __( '<h4>', 'turbolife'),
 'after_title' => __( '</h4>', 'turbolife'),
) );
genesis_register_sidebar( array(
 'id'     => 'optin-after-entry',
 'name'    => __( 'Opt-in After Entry', 'turbolife' ),
 'description' => __( 'This is the opt-in form after a single entry.', 'turbolife' ),
 'before_title'=> __( '<h4>', 'turbolife'),
 'after_title' => __( '</h4>', 'turbolife'),
) );